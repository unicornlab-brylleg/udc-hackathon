...
export default class StreamRenderer extends React.Component {
  ...
  async componentDidMount() {
    ...
    this.stream.on("isAvailableChanged", async () => {
      try {
        ...
        console.log(
          `[App][StreamMedia][id=${this.stream.id}][isAvailableChanged] triggered`
        );
        if (this.stream.isAvailable && !this.renderer) {
          console.log(
            `[App][StreamMedia][id=${this.stream.id}][isAvailableChanged] isAvailable=${this.stream.isAvailable}`
          );
    ...
    try {
      console.log(
        `[App][StreamMedia][id=${this.stream.id}] checking initial value - isAvailable=${this.stream.isAvailable}`
      );
      if (this.stream.isAvailable && !this.renderer) {
        await this.createRenderer();
        this.attachRenderer();
  ...
  async createRenderer() {
    console.info(
      `[App][StreamMedia][id=${this.stream.id}][renderStream] attempt to render stream type=${this.stream.mediaStreamType}, id=${this.stream.id}`
    );
    if (!this.renderer) {
      this.renderer = new VideoStreamRenderer(this.stream);
      this.view = await this.renderer.createView();
      console.info(
        `[App][StreamMedia][id=${this.stream.id}][renderStream] createView resolved, appending view`
      );
    } else {
      throw new Error(
  ...
  async attachRenderer() {
    console.info(
      `[App][StreamMedia][id=${this.stream.id}][attachRenderer] attempt to attach view=${this.view.target}, id=${this.stream.id} to DOM, under container id=${this.videoContainerId}`
    );
    try {
      if (!this.view.target) {
        throw new Error(
  ...
  disposeRenderer() {
    if (this.renderer) {
      ...
      console.warn(
        `[App][StreamMedia][id=${this.stream.id}][disposeRender] no renderer to dispose`
      );
    }
  }
